...
The collaborator mapping table, normal Edge Function handler, direct `pg_net` sender, and the single `public.projects` trigger are deployed to production. Version 11 of `odoo-project-sync` uses `index.ts` with JWT verification and the user-owned `ODOO_SYNC_ENABLED=true` environment gate. The visible source description is confirmed by screenshot. Version 11 resolves the current owner through the exact `projects.user → userData.uid → userData.id → res.partner.x_studio_mocklab_id` chain and writes the unique matched internal ID to `partner_id`. The sender adds no queue, Cron, dashboard feedback, catch-up, or automatic retry.
...
2. The active deployment is version 11 (`e15c0a47-e8d6-4637-8eec-cc249eb38c4c`), status `ACTIVE`, with `verify_jwt=true`, entrypoint `index.ts`, and bundle SHA-256 `fa16b1b68821d15ae04e1c674ede41329b7358fe93c257af6a47ab4e528a0487`. Fresh independent readback confirmed the authentication guard, exact enable gate, optional database-header behavior, dual description mapping, and fail-closed customer helper before project or child writes; DELETE remains independent. The bundle SHA identifies the deployed bundle, not a byte-hash proof of the reviewed source. `ACTIVE` means hosting is ready, not that synchronization is enabled or Odoo is verified.
...
1. The normal handler requires these Edge Function secrets: `ODOO_SYNC_ENABLED=false` initially, `ODOO_JSON2_URL` (clean HTTPS origin), `ODOO_API_KEY`, `ODOO_PROJECT_SYNC_WORKER_SECRET`, and optionally `ODOO_DATABASE`, `ODOO_CATEGORY_MAPPINGS` (JSON source category slug to positive Odoo ID), and `ODOO_SYNC_EXTRA_IMAGE_HOSTS` (comma-separated public HTTPS hosts). The user reports the requested secrets are configured; this has not been independently read or verified, and no values are recorded. Set `ODOO_DATABASE` only when the Odoo host requires database selection; when omitted or blank, the function omits `X-Odoo-Database`. `SUPABASE_URL` and the matching legacy `SUPABASE_SERVICE_ROLE_KEY` are runtime values. DELETE needs only the Odoo connection/authentication settings; category and image settings are evaluated only for a live source row.
2. Populate collaborator mappings only for existing Odoo contacts. Missing mappings block the whole INSERT/UPDATE before Odoo writes. The function never guesses, creates, or changes `res.partner` records.
3. Each INSERT/UPDATE resolves the project customer exactly as `projects.user → userData.uid → userData.id → res.partner.x_studio_mocklab_id`, then writes the matched Odoo internal `res.partner.id` to `project.project.partner_id`. `userData.odoo_id` is not used. The partner lookup requests only `id` and `x_studio_mocklab_id`, includes archived records only to detect duplicate identities, and fails closed for a missing owner/account/customer, duplicate match, or returned-ID mismatch. It never selects an archived contact by a guessed activity field.
...
6. In **Dashboard → Edge Function Secrets**, check and set `ODOO_SYNC_ENABLED=true` before the controlled edit. The approved scope is unfiltered future synchronization with normal project titles; the function has no title-prefix filter.
7. Make an actual source-field change to project 421 in Mocklab and save it, then refresh Odoo and check the customer. Do not manually write Odoo, create another Dashboard UI webhook, manually invoke HTTP, enqueue a request, or replay the project. Use no collaborators while the mapping table is empty, and do not use binary images.
8. After the edit, the parent checks net HTTP metadata and logs without exposing secrets. The pilot does not backfill events: retry a project update if a past INSERT was never captured. Real Odoo CREATE, UPDATE, and DELETE verification remains pending.
...
| Event | Behavior |
| --- | --- |
| INSERT / UPDATE | Ignores snapshot business fields and rereads the current `public.projects` row by ID with service role. It resolves the current owner to one exact customer and writes its internal Odoo ID as `partner_id`; cloned projects naturally use their current owner. It searches projects by exact `x_mocklab_id`, including archived records: zero creates, one writes supplied fields, more than one fails. A missing current row follows the delete path. |
...
- Invalid input, missing configuration, owner/customer mapping failures, unmapped categories/collaborators, duplicate remote identities, and ambiguous owned child markers fail with a non-2xx response and a redacted backend log containing correlation ID, event type, project ID where available, and a stable reason code. Response bodies remain generic.
...
- The prior project 421 update was reported at `2026-09-21T18:24:39.195Z`; `pg_net` request 1 returned HTTP 200 `synced` at `2026-09-21T18:24:39.999Z` with correlation `b2a7a9b7-fd6c-4300-bbb2-f74c08c79bcb`. This confirms one authenticated delivery, not visible-field mapping or full Odoo end-to-end validation.
